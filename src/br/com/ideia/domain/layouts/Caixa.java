...
public class Caixa extends AbstractBanco implements Banco {
	...
	public String geraCodigoDeBarrasPara(Boleto boleto) {
		...
		if (carteiraDoEmissor.equals("1")) {
			...
			String nossoNumeroCompleto = getNossoNumeroFormatado(beneficiario);
			campoLivre.append(leftPadWithZeros(beneficiario.getCodigoBeneficiario(), 6));//OK
			campoLivre.append(beneficiario.getDigitoCodigoBeneficiario());//OK
			campoLivre.append(nossoNumeroCompleto.substring(2, 5));//3 a 5a posição do nosso numero
			campoLivre.append(nossoNumeroCompleto.substring(0, 1));//1a posição do nosso numero
			campoLivre.append(nossoNumeroCompleto.substring(5, 8));//6a a 8a posição do nosso numero
			campoLivre.append(nossoNumeroCompleto.substring(1, 2));//2a posição do nosso numero
			campoLivre.append(nossoNumeroCompleto.substring(8)); // 9a a 17a posição do nosso numero
			campoLivre.append(geradorDeDigito.geraDigitoMod11AceitandoRestoZero(campoLivre.toString()));
		} else if (carteiraDoEmissor.equals("14")) {
			String nossoNumeroCompleto = getNossoNumeroFormatado(beneficiario);
			campoLivre.append(leftPadWithZeros(beneficiario.getCodigoBeneficiario(), 6));//OK
			campoLivre.append(beneficiario.getDigitoCodigoBeneficiario());//OK
			campoLivre.append(nossoNumeroCompleto.substring(2, 5));//3 a 5a posição do nosso numero
	...
	public String getNossoNumeroFormatado(Beneficiario beneficiario) {
		return getCarteiraFormatado(beneficiario) +  leftPadWithZeros(beneficiario.getNossoNumero(), 15);
	}
